...
**`perception.py`** and **`grasp.py`** are the stateless halves of the node's vision/grasp math, extracted as plain module functions that take their dependencies (Gemini client, SAM2 predictor, camera_info, transforms) as explicit arguments — so they carry no ROS-node state and are testable in isolation:
  ...
  - `grasp.py`: `GRIPPER_MAX_WIDTH_M`/`GRIPPER_FULLY_CLOSED_THRESHOLD`/`R_ALIGN`/`GRASP_STANDOFF_M`/`MAX_INSERTION_M` constants, `width_to_gripper_percent`, `tool_z_axis`/`shift_along_tool_z` (approach-axis geometry, mirroring `Controller::toolZAxis` — the euler convention must match on both sides), `insertion_depth_m`, `build_anygrasp_clouds`, `anygrasp_grasp_to_base_pose`.
